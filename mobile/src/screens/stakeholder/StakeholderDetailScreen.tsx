...
export default function StakeholderDetailScreen({ route, navigation }: any) {
  ...
  const mapSurveyCamel = (row: any) => {
    ...
    return {
      ...
      email: row.email,
      contactPerson2: row.contact_person_2 ?? row.contactPerson2,
      mobileNumber2: row.mobile_number_2 ?? row.mobileNumber2,
      email2: row.email_2 ?? row.email2,
      website: row.website,
        ...
        {survey && (
          <CollapsibleSection title="Survey Data" icon="clipboard-text-outline" index={3} defaultExpanded={true}>
            {renderInfoRows([
              ...
              { label: 'Contact Person (Alt)', value: survey.contactPerson2 },
              { label: 'Mobile (Alt)', value: survey.mobileNumber2 },
              { label: 'Email (Alt)', value: survey.email2 },
